...
func (l *GetMenuListByRoleIdLogic) GetMenuListByRoleId(in *pb.IDReq) (*pb.MenuInfoList, error) {
	...
	for _, m := range menuList {
		...
		resp.MenuInfo = append(resp.MenuInfo, &pb.MenuInfo{
			...
			Permission:  fun.NullStringToPtr(m.Permission),
			CreatedAt:   fun.Int64ToUint64Ptr(m.CreatedAt.UnixMilli()),
			UpdatedAt:   fun.Int64ToUint64Ptr(m.UpdatedAt.UnixMilli()),
			Meta: &pb.Meta{
				Title:              &m.Title,
				Icon:               &m.Icon,
				HideMenu:           fun.IntToBoolPtr(m.HideMenu),
				HideBreadcrumb:     fun.IntToBoolPtr(m.HideBreadcrumb),
				IgnoreKeepAlive:    fun.IntToBoolPtr(m.IgnoreKeepAlive),
				HideTab:            fun.IntToBoolPtr(m.HideTab),
				FrameSrc:           &m.FrameSrc,
				CarryParam:         fun.IntToBoolPtr(m.CarryParam),
				HideChildrenInMenu: fun.IntToBoolPtr(m.HideChildrenInMenu),
				Affix:              fun.IntToBoolPtr(m.Affix),
